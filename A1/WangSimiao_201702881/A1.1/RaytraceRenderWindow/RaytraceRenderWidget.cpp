...
//                    }
                ...
                /* -- task3 -- */
                if(!renderParameters->shadowsEnabled && !renderParameters->phongEnabled && !renderParameters->interpolationRendering && !renderParameters->reflectionEnabled){
                    //射线碰到了物体, 返回白色
...
//    std::cout<<"normal"<<ray.direction.x<< " "<<ray.direction.y<<" "<<ray.direction.z<<std::endl;
//    std::cout<<"tri.point"<<closestTri.tri.verts[0].x<<" "<<closestTri.tri.verts[0].y<<" "<<closestTri.tri.verts[0].z<<std::endl;
    //重心坐标插值 P
